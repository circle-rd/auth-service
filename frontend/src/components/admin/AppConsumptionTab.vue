...
<template>
    <div class="space-y-4">
        <!-- Search -->
        <input v-model="search" type="search" placeholder="Rechercher par utilisateur ou clé…" class="input w-full" />

        <div v-if="loading" class="text-center py-8 text-sm" style="color: var(--text-muted)">
            Chargement…
        </div>

        <div v-else-if="filtered.length === 0" class="text-center py-8 text-sm" style="color: var(--text-muted)">
            {{ search ? "Aucun résultat." : "Aucune donnée de consommation." }}
        </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    <tr v-for="entry in filtered" :key="`${entry.userId}-${entry.key}`"
                        ...
                        <td class="px-3 py-2.5">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs" style="color: var(--text-muted)">{{ entry.userEmail ??
                                        entry.userId }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-3 py-2.5 text-right font-mono font-semibold" style="color: var(--accent-cyan)">
                            {{ Number(entry.total).toLocaleString("fr-FR") }}
                        </td>
                        <td class="px-3 py-2.5 text-right text-xs" style="color: var(--text-muted)">
