    ...
    <div class="space-y-6">
        ...
        <div class="flex items-start gap-4">
            ...
            <div v-if="loading" class="flex-1">
                <div
                    style="height: 1.5rem; width: 10rem; background: var(--color-border); border-radius: 4px; animation: pulse 1.5s ease-in-out infinite" />
            </div>
            ...
            <div v-else class="text-sm" style="color: var(--color-text-muted)">Application introuvable</div>
        </div>
        ...
        <div v-if="app" class="card">
            <div class="grid gap-5" style="grid-template-columns: repeat(auto-fill, minmax(14rem, 1fr))">
                <div v-if="app.appUrl">
                    <p class="text-xs font-medium mb-1" style="color: var(--color-text-muted)">URL de l'application</p>
                ...
                <div v-if="app.description">
                    ...
                    <p class="text-sm" style="color: var(--color-text)">{{ app.description }}</p>
                </div>
                <div>
                    <p class="text-xs font-medium mb-1" style="color: var(--color-text-muted)">Created</p>
        ...
        <div v-if="app" class="grid gap-3" style="grid-template-columns: repeat(auto-fill, minmax(10rem, 1fr))">
            <div class="card !p-4">
                <p class="text-xs font-medium mb-1" style="color: var(--color-text-muted)">Users</p>
            ...
            <div class="card !p-4">
                <p class="text-xs font-medium mb-1" style="color: var(--color-text-muted)">Client ID</p>
                <p class="text-sm font-mono truncate" style="color: var(--color-primary)">{{ app.slug }}</p>
            </div>
        </div>
        ...
        <div v-if="app" class="grid gap-4" style="grid-template-columns: repeat(auto-fill, minmax(12rem, 1fr))">
            <RouterLink :to="`/admin/applications/${appId}/roles`"
                class="card flex items-center gap-3 !p-4 hover:border-[color:var(--color-primary)] transition-colors"
                style="border-color: var(--color-border)">
                <ShieldCheck class="w-5 h-5 shrink-0" style="color: var(--color-primary)" />
                <div>
                    <p class="text-sm font-medium">{{ t("admin.roles") }}</p>
                    <p class="text-xs mt-0.5" style="color: var(--color-text-muted)">Rôles &amp; permissions</p>
                </div>
            </RouterLink>
            <RouterLink :to="`/admin/applications/${appId}/plans`"
                class="card flex items-center gap-3 !p-4 hover:border-[color:var(--color-primary)] transition-colors"
                style="border-color: var(--color-border)">
                <CreditCard class="w-5 h-5 shrink-0" style="color: var(--color-primary)" />
                <div>
                    <p class="text-sm font-medium">{{ t("admin.plans") }}</p>
                    <p class="text-xs mt-0.5" style="color: var(--color-text-muted)">Subscriptions</p>
                </div>
            </RouterLink>
            <RouterLink :to="`/admin/applications/${appId}/users`"
                class="card flex items-center gap-3 !p-4 hover:border-[color:var(--color-primary)] transition-colors"
                style="border-color: var(--color-border)">
                <Users class="w-5 h-5 shrink-0" style="color: var(--color-primary)" />
                <div>
                    <p class="text-sm font-medium">{{ t("admin.appUsers") }}</p>
                    <p class="text-xs mt-0.5" style="color: var(--color-text-muted)">User access</p>
                </div>
            </RouterLink>
            <RouterLink :to="`/admin/applications/${appId}/usage`"
                class="card flex items-center gap-3 !p-4 hover:border-[color:var(--color-primary)] transition-colors"
                style="border-color: var(--color-border)">
                <BarChart3 class="w-5 h-5 shrink-0" style="color: var(--color-primary)" />
                <div>
                    <p class="text-sm font-medium">{{ t("admin.usage") }}</p>
                    <p class="text-xs mt-0.5" style="color: var(--color-text-muted)">API consumption</p>
                </div>
            </RouterLink>
            <RouterLink :to="`/admin/applications/${appId}/integration`"
                class="card flex items-center gap-3 !p-4 hover:border-[color:var(--color-primary)] transition-colors"
                style="border-color: var(--color-border)">
                <Puzzle class="w-5 h-5 shrink-0" style="color: var(--color-primary)" />
                <div>
                    <p class="text-sm font-medium">{{ t("admin.integration") }}</p>
                    <p class="text-xs mt-0.5" style="color: var(--color-text-muted)">Exemples OAuth</p>
                </div>
            </RouterLink>
        </div>
