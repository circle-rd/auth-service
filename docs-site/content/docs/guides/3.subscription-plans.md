...
A user's subscription is part of their application access record. Set it with a
`PATCH` on that record — there is no separate subscription endpoint.
...
Set `subscriptionPlanId` to `null` on the same access record:
