...
| `profile` | `name`, `picture`, `updated_at`, `company` | Basic user profile. |
| `email` | `email`, `email_verified` | User email address. |
| `phone` | `phone_number` | User phone number (if configured). |
...
| `org` | `org_id` | Active organization ID (multi-tenant). |
