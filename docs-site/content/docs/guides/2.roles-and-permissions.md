...
```sh
# Assign — permissionId is sent in the request body
...
A user's role for an application is part of their access record. Set it when
granting access, or update it afterwards — there is no separate role-assignment
endpoint.
